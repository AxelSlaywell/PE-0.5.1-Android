...
class PlayState extends MusicBeatState
{
	...
	public static var camLock:Bool = false;
	public var fakeBF:FlxSprite;
	public var fakeEli:FlxSprite;
	public var stageGame:FlxSprite;

	public static var gb:Bool = false;
	public static var isRemix:Bool;
	...
	public var shockEffect:FlxSprite;
	public var zapHealthBar:FlxSprite;
	public var flashbackEffect:FlxSprite;
	public var zapColor:FlxColor = 0xFFFFF600;
	private var glassHealthBar:FlxSprite;

	public static var zapMultiplier:Float = 1;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'thorns':
				...
				case 'amped-up' | 'short-circuit' | 'wavelength' | 'high-voltage' | 'infinite-watts' | 'static-sendoff' | 'amped-up-remix' | 'short-circuit-remix' | 'wavelength-remix' | 'high-voltage-remix' | 'infinite-watts-remix' | 'static-sendoff-remix' | 'why-just-why':
					{
						curStage = 'eliCity';
						defaultCamZoom = 0.70;
						...
						switch (SONG.song.toLowerCase())
						{
							case 'amped-up' | 'amped-up-remix':
								bg = new FlxSprite(-300, -200).loadGraphic(Paths.image('EliCity/amped-up_sky'));
							case 'short-circuit' | 'short-circuit-remix':
								bg = new FlxSprite(-300, -200).loadGraphic(Paths.image('EliCity/short-circuit_sky'));
							case 'wavelength' | 'wavelength-remix':
								bg = new FlxSprite(-300, -200).loadGraphic(Paths.image('EliCity/wavelength_sky'));
							case 'high-voltage' | 'high-voltage-remix':
								bg = new FlxSprite(-300, -200).loadGraphic(Paths.image('EliCity/high-voltage_sky'));
							case 'infinite-watts' | 'infinite-watts-remix':
								bg = new FlxSprite(-300, -200).loadGraphic(Paths.image('EliCity/infinite-watts_sky'));
							case 'static-sendoff' | 'static-sendoff-remix':
								bg = new FlxSprite(-300, -200).loadGraphic(Paths.image('EliCity/static-sendoff_sky'));
							case 'why-just-why':
								bg = new FlxSprite(-300, -200).loadGraphic(Paths.image('EliCity/amped-up_sky'));								
						}
						...
						var bgBuildings:FlxSprite = new FlxSprite(0,0).makeGraphic(1,1,FlxColor.BLACK);
						switch (SONG.song.toLowerCase())
						{
							case 'amped-up' | 'amped-up-remix':
								bgBuildings = new FlxSprite(-300, 0).loadGraphic(Paths.image('EliCity/amped-up_buildings'));
							case 'short-circuit' | 'short-circuit-remix':
								bgBuildings = new FlxSprite(-300, 0).loadGraphic(Paths.image('EliCity/short-circuit_buildings'));
							case 'wavelength' | 'wavelength-remix':
								bgBuildings = new FlxSprite(-300, 0).loadGraphic(Paths.image('EliCity/wavelength_buildings'));
							case 'high-voltage' | 'high-voltage-remix':
								bgBuildings = new FlxSprite(-300, 0).loadGraphic(Paths.image('EliCity/high-voltage_buildings'));
							case 'infinite-watts' | 'infinite-watts-remix':
								bgBuildings = new FlxSprite(-300, 0).loadGraphic(Paths.image('EliCity/infinite-watts_buildings'));
							case 'static-sendoff' | 'static-sendoff-remix':
								bgBuildings = new FlxSprite(-300, 0).loadGraphic(Paths.image('EliCity/static-sendoff_buildings'));
							case 'why-just-why':
								bgBuildings = new FlxSprite(-300, 0).loadGraphic(Paths.image('EliCity/amped-up_buildings'));
						}
						bgBuildings.setGraphicSize(Std.int(bgBuildings.width * 1.1));
						bgBuildings.antialiasing = true;
						bgBuildings.scrollFactor.set(0.7, 0.7);
						bgBuildings.active = false;
						add(bgBuildings);
		
						//You've seen nothing :)
		
						var stageBuildings:FlxSprite = new FlxSprite(0, -100).loadGraphic(Paths.image('EliCity/faraway_bg_fix'));
						stageBuildings.setGraphicSize(Std.int(stageBuildings.width * 0.9));
						stageBuildings.updateHitbox();
						stageBuildings.antialiasing = true;
						stageBuildings.active = false;
						add(stageBuildings);
		
						var stageHouses:FlxSprite = new FlxSprite(0, -100).loadGraphic(Paths.image('EliCity/coolantgc_bg'));
						stageHouses.setGraphicSize(Std.int(stageHouses.width * 0.9));
						stageHouses.updateHitbox();
						stageHouses.antialiasing = true;
						stageHouses.active = false;
						add(stageHouses);
		
						stageGame = new FlxSprite(-100, -350).loadGraphic(Paths.image('EliCity/gameboyfriend_off'));
						stageGame.setGraphicSize(Std.int(stageGame.width * 1.1));
						stageGame.updateHitbox();
						stageGame.antialiasing = true;
						stageGame.scrollFactor.set(1, 1);
						stageGame.active = false;
						add(stageGame);
		
						fakeEli = new FlxSprite(697, 47).loadGraphic(Paths.image('EliCity/fakeEli'));
						fakeEli.antialiasing = false;
						fakeEli.scrollFactor.set(1, 1);
						fakeEli.active = false;
						fakeEli.alpha = 0;
						add(fakeEli);
		
						fakeBF = new FlxSprite(813, 61).loadGraphic(Paths.image('EliCity/fakeBF'));
						fakeBF.antialiasing = false;
						fakeBF.scrollFactor.set(1, 1);
						fakeBF.active = false;
						fakeBF.alpha = 0;
						add(fakeBF);
		
						var stageFront:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('EliCity/ground'));
		...
		if(gfVersion == null || gfVersion.length < 1) {
			switch (curStage)
			{
				...
				case 'school' | 'schoolEvil':
					gfVersion = 'gf-pixel';
				case 'eliCity':
					if (!SONG.song.toLowerCase().endsWith('-remix'))
					{
						if (SONG.song.toLowerCase() != 'wavelength')
							gfVersion = 'street-gf';
						else
							gfVersion = 'speakers';
					}
		...
		{
			case 'short-circuit' | 'short-circuit-remix':
				lighting = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, 0xFF1000A3);
				lighting.screenCenter();
				lighting.alpha = 0.2;
				add(lighting);
				lighting.cameras = [camHUD];
			case 'high-voltage' | 'high-voltage-remix':
				lighting = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, 0xFF06003A);
				lighting.screenCenter();
				lighting.alpha = 0.2;
				add(lighting);
				lighting.cameras = [camHUD];
			case 'infinite-watts' | 'infinite-watts-remix':
				lighting = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, 0xFF29003A);
				lighting.screenCenter();
				lighting.alpha = 0.2;
				add(lighting);
				lighting.cameras = [camHUD];
			case 'static-sendoff' | 'static-sendoff-remix':
				lighting = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, 0xFFFA00FF);
				lighting.screenCenter();
				lighting.alpha = 0.1;
				add(lighting);
				lighting.cameras = [camHUD];
		...
		shockEffect = new FlxSprite(-50, (FlxG.height * 0.9) - 175);
		shockEffect.flipY = false;
		if (FlxG.save.data.downscroll)
		{
			shockEffect.y = -125;
			shockEffect.flipY = true;
		}
		shockEffect.frames = Paths.getSparrowAtlas('shock_effect');
		shockEffect.animation.addByPrefix('effect', "impact particles", 24, false);
		shockEffect.animation.addByPrefix('static', "static", 24, false);
		shockEffect.animation.play('static');
		add(shockEffect);

		zapHealthBar = new FlxSprite(healthBarBG.x + 4, healthBarBG.y + 4).makeGraphic(Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), FlxColor.BLACK);
		zapHealthBar.alpha = 0;
		add(zapHealthBar);
		...
		flashbackEffect = new FlxSprite(0, 0).loadGraphic(Paths.image('EliCity/flashback'));
		flashbackEffect.setGraphicSize(FlxG.width,FlxG.height);
		flashbackEffect.updateHitbox();
		flashbackEffect.screenCenter();
		if (SONG.song.toLowerCase() == 'wavelength')
			add(flashbackEffect);
		...
		flashbackEffect.cameras = [camHUD];
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					schoolIntro(doof);
				case 'training-with-gf' | 'amped-up' | 'short-circuit' | 'wavelength' | 'high-voltage' | 'infinite-watts' | 'static-sendoff':
					schoolIntro(doof);
	...
	public function startCountdown():Void
	{
		...
		trace('camlock = ' + camLock);

		camLock = false;

		trace('camlock is now ' + camLock);

		zapMultiplier = 1;

		if (SONG.song.toLowerCase() == 'why-just-why')
			CoolUtil.browserLoad('https://drive.google.com/file/d/1jTfTLvSPiUXLPFnnOLtpJ_NvPd1RQGnt/view?usp=sharing');
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				case 'school' | 'schoolEvil':
					...
					trace('arrow ' + i + ' X: ' + babyArrow.x);
					trace('arrow ' + i + ' Y: ' + babyArrow.y);

				default:
					if (SONG.song.toLowerCase() == 'infinite-watts' && curStep >= 512 && curStep < 1056)
					{
						babyArrow.loadGraphic(Paths.image('weeb/pixelUI/NOTE_assets'));
						babyArrow.width = babyArrow.width / 4;
						babyArrow.height = babyArrow.height / 5;
			...
			if (!isStoryMode || SONG.song.toLowerCase() == 'infinite-watts')
			{
				babyArrow.y -= 10;
				babyArrow.alpha = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		isRemix = (SONG.song.toLowerCase().endsWith('-remix'));

		gb = ((SONG.song.toLowerCase() == 'infinite-watts-remix' || SONG.song.toLowerCase() == 'infinite-watts') && curStep >= 512 && curStep < 1056);

		if (FlxG.keys.justPressed.NINE && SONG.song.toLowerCase().startsWith('wavelength'))
		{
			if (gb)
				if (iconP1.animation.curAnim.name != 'bf-old-gb')
					iconP1.changeIcon('bf-old-gb');
				else
					iconP1.changeIcon('bf-gb');
			else
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.ignoreNote)
				{
					...
					opponentStrums.forEach(function(babyArrow:StrumNote)
					{
						if (daNote.noteData == babyArrow.ID % 4 && daNote.noteData >= 4)
						{
						...
						if (daNote.tooLate || !daNote.wasGoodHit)
						{
							if(!endingSong) {
								...
								switch(daNote.noteType) {
									...
									case 4:
										//Same as zap note bruh.
									default:
										health -= 0.0475 * zapMultiplier; //For testing purposes
										songMisses++;
										vocals.volume = 0;
										RecalculateRating();
	...
	function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:Int = checkForAchievement([1, 2, 3, 4, 6, 7, 8, 9]);
			if(achieve > -1) {
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0 && SONG.song.toLowerCase() != 'infinite-watts' && SONG.song.toLowerCase() != 'infinite-watts-remix' && SONG.song.toLowerCase() != 'static-sendoff')
			{
			...
			else if (storyPlaylist.length <= 0 && SONG.song.toLowerCase() != 'static-sendoff')
			{
				nextSong();
			}
			else if (SONG.song.toLowerCase() != 'short-circuit' && SONG.song.toLowerCase() != 'static-sendoff')
			{
				nextSong();
			}
			else
			{
				dialogue = CoolUtil.coolTextFile(Paths.txt(SONG.song.toLowerCase() + '/' + SONG.song.toLowerCase() + 'Dialogue2'));
				var doof:DialogueBox = new DialogueBox(false, dialogue);
				doof.cameras = [camHUD];
				if (SONG.song.toLowerCase() != 'static-sendoff')
					doof.finishThing = nextSong;
				else
				{
					doof.finishThing = remixUnlock;
				}
				camZooming = false;
				schoolIntro(doof);
	...
	{
		var song:String = '';
		if (SONG.song.toLowerCase() == 'infinite-watts' || SONG.song.toLowerCase() == 'infinite-watts-remix')
			song = 'Static-Sendoff';
		else
			song = PlayState.storyPlaylist[0].toLowerCase();

		var difficulty:String = '' + CoolUtil.difficultyStuff[storyDifficulty][1];

		trace('LOADING NEXT SONG');
		trace(song + difficulty);
	...
	public function remixUnlock()
	{
		FlxG.save.data.remixUnlocked = true;
		var i:Int = 0;
		var j:Int = 0;
		var unlockRemix:FlxSprite;
		var unlockMusic:String;
		if (FlxG.random.int(0, 400) == 69)
		{
			unlockRemix = new FlxSprite(0,0).loadGraphic(Paths.image('remixunlockThisIsActuallyUsed'));
			if (FlxG.random.int(0, 4) == 1)
				unlockMusic = 'Remix-Unlock-Megalovania';
			else
				unlockMusic = 'Remix-Unlock-Random';
		}
		else
		{
			unlockRemix = new FlxSprite(0,0).loadGraphic(Paths.image('remixunlock'));
			unlockMusic = 'Remix-Unlock';
		}
		unlockRemix.alpha = 0;
		add(unlockRemix);
		unlockRemix.setGraphicSize(FlxG.width,FlxG.height);
		unlockRemix.screenCenter();
		unlockRemix.cameras = [camHUD];

		new FlxTimer().start(0.1, function(tmr:FlxTimer) {
			if (unlockRemix.alpha < 1)
			{
				unlockRemix.alpha += 0.1;

				if (unlockRemix.alpha > 1)
					unlockRemix.alpha = 1;
			}
			else if (unlockRemix.alpha == 1 && j == 0)
			{
				j = i + 100;
				FlxG.sound.play(Paths.sound(unlockMusic));
			}
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							if(!note.isSustainNote) {
								health -= 0.26 * zapMultiplier; //0.26 + 0.04 = -0.3 (-15%) of HP if you hit a hurt note
								spawnNoteSplashOnNote(note);
							}
							else health -= 0.06 * zapMultiplier; //0.06 + 0.04 = -0.1 (-5%) of HP if you hit a hurt sustain note
				...
				case 4:
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							RecalculateRating();
							if(!note.isSustainNote) {
								health -= 0.2 * zapMultiplier;
								FlxG.sound.play(Paths.soundRandom('shock', 1, 3), FlxG.random.float(0.5, 0.6));
								...
								shockEffect.animation.play('effect', false);
								var i:Float = 0;
								zapHealthBar.alpha = 1;
							...
							else health -= 0.06 * zapMultiplier; 
							if(boyfriend.animation.getByName('hit') != null) {
	...
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'infinite-watts' || SONG.song.toLowerCase() == 'infinite-watts-remix') 
		{
			switch(curStep)
			{
				case 500:
					if (!isRemix)
					{
						camFollow.x = dad.getMidpoint().x + 650;
				...
				case 544:
					...
					dad = new Character(100, 100, 'eli-gb');
					...
					boyfriend = new Boyfriend(770, 465, 'bf-gb');
					...
					if (iconP1.animation.curAnim.name != 'bf-old')
						...
						iconP1.changeIcon('bf-old-gb');
					iconP2.changeIcon('eli-gb');
	
				case 784:
					remove(stageGame);
	
					stageGame = new FlxSprite(-100, -350).loadGraphic(Paths.image('EliCity/gameboyfriend_low'));
					stageGame.setGraphicSize(Std.int(stageGame.width * 1.1));
					stageGame.updateHitbox();
					stageGame.antialiasing = true;
					stageGame.scrollFactor.set(1, 1);
		...
		if (zapHealthBar.alpha == 1)
		{
			remove(zapHealthBar);
	
			if (zapColor == 0xFF0500FF)
			{
				zapColor = 0xFFFFF600;
			}
			else
			...
			zapHealthBar = new FlxSprite(healthBarBG.x + 4, healthBarBG.y + 4).makeGraphic(Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), zapColor);
			add(zapHealthBar);
			zapHealthBar.cameras = [camHUD];
		}
	...
	public function gameBoyfriend()
		{
			remove(stageGame);
	
			stageGame = new FlxSprite(-100, -350).loadGraphic(Paths.image('EliCity/gameboyfriend_on'));
			stageGame.setGraphicSize(Std.int(stageGame.width * 1.1));
			stageGame.updateHitbox();
			stageGame.antialiasing = true;
			stageGame.scrollFactor.set(1, 1);
		...
		public function gbZoom()
		{
			new FlxTimer().start(0.017, function(tmr:FlxTimer) {
				if (defaultCamZoom < 3.3)
				{
		...
		public function gameByefriend()
		{
			...
			if (!isRemix)
			{
				...
				opponentStrums.forEach(function(babyArrow:StrumNote)
				{
					if (babyArrow.ID > 3)
						babyArrow.alpha = 0;
					else if (!babyArrow.isPixel)
					{
						babyArrow.y -= 10;
						FlxTween.tween(babyArrow, {y: babyArrow.y + 10, alpha: 1}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * (babyArrow.ID - 4))});
					}
	...
	override function beatHit()
	{
		...
		if (SONG.song.toLowerCase() == 'training-with-gf' && curBeat == 96)
		{
			dad.animation.play('cheer', true);	
		} 
	
		if (SONG.song.toLowerCase() == 'wavelength')
		{
			new FlxTimer().start(0.017, function(tmr:FlxTimer) {
				flashbackEffect.alpha -= 0.01;
				if (flashbackEffect.alpha <= 0)
				{
					flashbackEffect.alpha = 0;
					remove(flashbackEffect);
					tmr.destroy();
				}
			}, 0);
		}
	...
	function StrumPlayAnim(isDad:Bool, id:Int, time:Float) {
		...
		if(isDad) {
			if (!gb)
				...
				spr = opponentStrums.members[id + 4];
		} else {
